...
def _build_agent_input_for_infra_safe(
    ...
    instructions.extend([
        ...
        "TEAMS CHANGED-FILES ONLY (HARD): return files[] only for repo-relative files whose final content differs from the live repository baseline. Do not include unchanged evidence files, context files, or companion files unless their content is actually modified by this request.",
        "TEAMS NO EXISTING-CODE REMOVAL (HARD): never delete, omit, collapse, summarize, or replace existing HCL/tfvars objects, list entries, containers, storage account maps, module blocks, variables, comments, or blank-line structure unless the current user explicitly requested deletion/removal of that exact content.",
        "TEAMS EXISTING-FILE BASELINE (HARD): for every existing file you return, construct the candidate by starting with the exact live file content supplied by backend evidence and applying the smallest requested delta. If your candidate is much shorter than the live file or drops unrelated blocks, self-reject before returning.",
    ])
