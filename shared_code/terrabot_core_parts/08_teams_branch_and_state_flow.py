...
def _teams_coerce_agent_payload_stage1(agent_text: str, context: dict) -> tuple[dict, dict]:
    ...
    def _expected_single_target_path() -> str:
        ...
        selected = next((
            ...
            and entry.get("source") == "backend_aws_selected_module_generation_context"
        ), {})
        target = str(selected.get("target_file") or selected.get("path") or "").strip()
        if target:
            return target
    ...
    while index < len(flattened_entries):
        ...
        if not isinstance(content, str):
            ...
            if isinstance(alt, str):
                content = alt
        if not filename and isinstance(content, str) and content.strip():
            # Content-only entry: bind it to the workflow's single deterministic
            # write target instead of silently discarding the generation.
            target = _expected_single_target_path()
            if target:
                filename = target
                coerced_filename_defaults += 1
    ...
    if dropped_non_dict or coerced_json_strings or coerced_hcl_strings or coerced_filename_defaults:
        _teams_diag_log(
    ...
    if not normalized_files:
        ...
        _teams_diag_log(
            ...
            level="warning",
            raw_files_count=len(raw_file_entries),
            normalized_files_count=len(normalized_files),
            dropped_files_count=max(0, len(flattened_entries) - len(normalized_files)),
            dropped_reason=" | ".join(dropped_details)[:1800],
            payload_mode=str(payload.get("mode") or ""),
            payload_workflow=str(payload.get("workflow") or ""),
...
def repair_and_parse_agent_output(
    ...
    repair_payload = {
        ...
        "operation_state": (
            "boolean" if isinstance(context.get("resolved_repository_target_contract"), dict) and context.get("resolved_repository_target_contract")
            else "create" if str(context.get("operation_state") or "").lower() == "create"
            else "modify"
        ),
    ...
    # Shape/protocol failures get one dedicated repair. Repeating semantic
