...
# ---------------------------------------------------------------------------
# Foundry input budgeting (fix for context_length_exceeded, run 2026-09-04)
# ---------------------------------------------------------------------------
_TEAMS_FOUNDRY_INPUT_BUDGET_DEFAULT = 240_000
_TEAMS_FOUNDRY_ITEM_CONTENT_CAP = 12_000
_TEAMS_FOUNDRY_TRUNCATION_NOTE = "\n# ...[truncated by Terrabot backend input budget; full file remains authoritative on GitHub]"
# Trimming order: least critical evidence first. Live/selected target evidence
# ("existing_infrastructure", picker selection) is trimmed LAST so the exact
# file being modified always survives intact for as long as possible.
_TEAMS_FOUNDRY_TRIM_PRIORITY = (
...
def _teams_foundry_input_budget_chars() -> int:
    raw = os.getenv("TERRABOT_FOUNDRY_INPUT_BUDGET_CHARS", "")
    try:
        value = int(str(raw).strip())
    except (TypeError, ValueError):
        value = _TEAMS_FOUNDRY_INPUT_BUDGET_DEFAULT
    return max(60_000, min(value or _TEAMS_FOUNDRY_INPUT_BUDGET_DEFAULT, 600_000))


def _teams_trim_long_strings(value, cap: int):
    """Recursively cap oversized string leaves inside a JSON-like payload."""
    if isinstance(value, str):
        if len(value) > cap:
            return value[:cap] + _TEAMS_FOUNDRY_TRUNCATION_NOTE
        return value
    if isinstance(value, list):
        return [_teams_trim_long_strings(item, cap) for item in value]
    if isinstance(value, dict):
        return {key: _teams_trim_long_strings(item, cap) for key, item in value.items()}
    return value


def _teams_enforce_agent_input_budget(
    agent_input: str,
    *,
    thread: str = "",
    cloud: str = "",
    ...
    """Shrink an outgoing Foundry payload below the model context budget.

    Strategy (applied only when over budget):
      1. Cap every string leaf inside the known bulk-context lists.
      2. Drop trailing items from those lists, lowest-priority list first,
         always keeping at least the single highest-ranked item.
      3. As a last resort, cap every string leaf in the whole payload.
    Non-JSON inputs are hard-truncated with an explicit notice.
    """
    text = str(agent_input or "")
    budget = _teams_foundry_input_budget_chars()
    ...
    try:
        ...
        trimmed = text[:budget] + _TEAMS_FOUNDRY_TRUNCATION_NOTE
        _teams_diag_log(
            ...
            cloud=cloud,
            workflow=workflow,
            strategy="raw_truncation",
            input_chars_before=original_len,
            input_chars_after=len(trimmed),
            budget_chars=budget,
        )
        return trimmed
    ...
    # Pass 1: cap oversized content inside the bulk-context lists.
    for key in _TEAMS_FOUNDRY_TRIM_PRIORITY:
        if _size() <= budget:
            break
        if isinstance(payload.get(key), (list, dict)):
            payload[key] = _teams_trim_long_strings(
                payload[key], _TEAMS_FOUNDRY_ITEM_CONTENT_CAP
            )

    # Pass 2: drop trailing (lowest-relevance) items from the bulk lists.
    for key in _TEAMS_FOUNDRY_TRIM_PRIORITY:
        items = payload.get(key)
        while isinstance(items, list) and len(items) > 1 and _size() > budget:
            items.pop()

    # Pass 3: cap every string leaf payload-wide (keeps structure/keys intact).
    if _size() > budget:
        payload = _teams_trim_long_strings(payload, _TEAMS_FOUNDRY_ITEM_CONTENT_CAP)
    if _size() > budget:
        payload = _teams_trim_long_strings(payload, 4_000)

    payload["backend_input_budget"] = {
        "enforced": True,
        "budget_chars": budget,
        "note": (
            "Some repository context bodies were truncated by the backend to fit "
            "the model context window. Truncated excerpts are marked; treat live "
            "GitHub content as authoritative and never emit truncation markers "
...
def handle_chat_request(data: dict):
    ...
    # Fallback only, for repos where files are unexpectedly at root.
            ...
            #
            # Run ctx-20260904-120034-a813fa: azure_infra_modification sent a
            # 722,819-char input; the follow-up repair turns in the SAME
            # conversation then failed with context_length_exceeded. Enforce a
            # hard character budget on the outgoing payload (trim oversized
            # context bodies, then drop lowest-priority context items) so both
            # the first generation call and every later repair turn fit the
            # model context window.
            agent_input = _teams_enforce_agent_input_budget(
                agent_input,
