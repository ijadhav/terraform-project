...
function reportGitStep(notes, reporter, message) {
    notes.push(message);
    reporter?.(message);
}
function resetThreadIfNeeded(root, historyLength) {
...
async function getWorkspaceRoot() {
        ...
        if (gitApi?.repositories?.length) {
            if (activeUri?.scheme === "file") {
                const matchingRepo = gitApi.repositories
                    .filter(repo => activeUri.fsPath === repo.rootUri.fsPath ||
                    activeUri.fsPath.startsWith(`${repo.rootUri.fsPath}${path.sep}`))
                    .sort((left, right) => right.rootUri.fsPath.length -
                    left.rootUri.fsPath.length)[0];
                if (matchingRepo) {
    ...
    const activeDirectory = vscode.window.activeTextEditor?.document.uri.fsPath
        ? path.dirname(vscode.window.activeTextEditor.document.uri.fsPath)
        : vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
    if (!activeDirectory) {
        throw new Error("Open a Git repository before using Terrabot.");
    }
    const repositoryRoot = await runCommandOnce("git", [
        "-C",
        activeDirectory,
        "rev-parse",
        "--show-toplevel",
    ...
    if (!repositoryRoot.trim()) {
        throw new Error(`Terrabot could not locate a Git repository from ${activeDirectory}.`);
    }
    return repositoryRoot.trim();
}
function getConfig(name, fallback) {
    const value = vscode.workspace.getConfiguration("terrabot").get(name);
    return value === undefined || value === null || value === "" ? fallback : value;
}
function getCliPath() {
    return getConfig("cliPath", "terrabot");
}
function normalizeEndpoint(value) {
    return value.trim().replace(/\/$/, "");
}
function getAiEndpoint() {
    const configuredEndpoint = getConfig("aiEndpoint", process.env.TERRABOT_AI_ENDPOINT || DEFAULT_AI_ENDPOINT);
...
function getApiToken() {
    return getConfig("apiToken", process.env.TERRABOT_API_TOKEN || "").trim();
}
...
async function connectGitHubAccount() {
    ...
    if (stored) {
        const user = await githubJson("GET", "/user", stored);
        return { token: stored, account: user.login, source: "pat" };
    }
    const choice = await vscode.window.showInformationMessage("Terrabot needs your GitHub account to push branches and create pull requests.", { modal: true }, "Enter GitHub token");
    ...
    const user = await githubJson("GET", "/user", token);
...
async function disconnectGitHubAccount() {
    ...
    vscode.window.showInformationMessage("Terrabot removed the manually stored GitHub token. VS Code GitHub sessions are managed through the Accounts menu.");
}
// ── CLI subprocess helper (unchanged) ────────────────────────────────────────
function runTerrabot(args, cwd) {
    return new Promise((resolve, reject) => {
        const child = (0, child_process_1.spawn)(getCliPath(), args, { cwd, shell: false });
        let stdout = "";
        let stderr = "";
...
async function runGitAuthenticated(args, cwd, token) {
    ...
    return new Promise((resolve, reject) => {
        const child = (0, child_process_1.spawn)("git", authenticatedArgs, {
            cwd,
            shell: false,
            env: {
                ...process.env,
...
function isGitIndexWriteError(message) {
...
function isGitIndexMutatingCommand(args) {
...
async function gitIndexLockPath(root) {
    const raw = await runCommandOnce("git", ["rev-parse", "--git-path", "index.lock"], root);
    return path.isAbsolute(raw) ? raw : path.join(root, raw);
}
async function removeStaleGitIndexLock(root, minimumAgeMs = 1500) {
    try {
...
async function waitForGitIndexToSettle(root, timeoutMs = 12000) {
...
async function unresolvedGitFiles(root) {
    try {
...
let gitCommandQueue = Promise.resolve();
async function runGitCommandSerialized(args, cwd) {
    const previous = gitCommandQueue;
    let release;
    gitCommandQueue = new Promise(resolve => { release = resolve; });
    await previous;
    try {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                if (!isGitIndexWriteError(message)) {
                    throw err;
                }
                const unresolved = await unresolvedGitFiles(cwd);
                if (unresolved.trim()) {
                    throw new Error(`Git cannot update the repository index because unresolved merge conflicts remain:
${unresolved}
...
function isPullRequestPrompt(prompt) {
...
function isPurePullRequestPrompt(prompt) {
    if (!isPullRequestPrompt(prompt)) {
        return false;
    }
...
function isBranchPrompt(prompt) {
    return /\b(create|make|open|switch)\b.*\b(github\s+)?branch\b/i.test(prompt);
}
function isPureBranchPrompt(prompt) {
    if (!isBranchPrompt(prompt) || isPullRequestPrompt(prompt)) {
        return false;
    }
...
function isSamePullRequestPrompt(prompt) {
...
function isSeparatePullRequestPrompt(prompt) {
...
function requestedBranchName(prompt) {
    const patterns = [
    ...
    for (const pattern of patterns) {
        ...
        if (INVALID_BRANCH_NAME_WORDS.has(candidate.toLowerCase())) {
            continue;
        }
        return candidate;
    }
...
function changeSlugFromPrompt(prompt) {
    ...
    for (const action of ["disable", "enable", "remove", "delete", "update", "create"]) {
        if (words.includes(action) && !ordered.includes(action)) {
            ordered.push(action);
        }
    }
    return sanitizeBranchPart(ordered.slice(0, 4).join("-")) || "terrabot-change";
}
async function resolveWorkspaceUser(root) {
    try {
...
async function defaultBranchName(root, prompt) {
...
function githubJson(method, apiPath, token, body) {
...
async function resolveGitContext(root) {
    ...
    return { owner, repo, base, branch };
}
async function gitPathExists(root, gitPathName) {
    try {
...
async function isRebaseInProgress(root) {
    return (await gitPathExists(root, "rebase-merge")) || (await gitPathExists(root, "rebase-apply"));
}
async function captureWorkspaceSnapshot(root) {
    await vscode.workspace.saveAll(false);
...
async function restoreWorkspaceSnapshot(root, snapshot) {
...
async function abortInterruptedGitOperation(root, notes, reporter) {
...
async function uniqueFreshBranchName(root, requested) {
    ...
    while (true) {
        ...
        if (!local.trim() && !remoteExists) {
            return candidate;
        }
        candidate = `${requested}-${suffix++}`;
    }
}
async function createBranchFromWorkspace(root, prompt, reporter) {
    const initialContext = await resolveGitContext(root);
    let requested = requestedBranchName(prompt);
    if (!requested) {
        requested = await defaultBranchName(root, prompt);
    }
...
async function ensureWorkspaceChangeBranch(root, prompt = "", forceSeparate = false, reporter) {
    ...
    const action = await vscode.window.showWarningMessage(forceSeparate
        ? `A separate pull request requires a separate source branch. Create one after updating from origin/${context.base}?`
        : `Before pushing, do you want Terrabot to create and switch to a new branch after updating from origin/${context.base}?`, { modal: true }, ...choices);
...
async function preserveWorkspaceChanges(root, notes, reporter) {
    await vscode.workspace.saveAll(false);
    ...
    reportGitStep(notes, reporter, "Preserved pre-existing local/workspace changes in a Git stash so they are not included in the new Terrabot branch.");
}
async function createFreshBranchBeforeGeneration(root, prompt, requestedName, reporter) {
    const notes = [];
    await abortInterruptedGitOperation(root, notes, reporter);
    ...
    await runCommand("git", ["checkout", "-b", branch, `origin/${GITHUB_BASE_BRANCH}`], root);
    reportGitStep(notes, reporter, `Created ${branch} from the latest origin/${GITHUB_BASE_BRANCH} before generating infrastructure changes.`);
    return { branch, notes, mode: "new", prTarget: "new" };
}
async function prepareGitTargetBeforeGeneration(root, prompt, wantsPullRequest, wantsBranch, reporter) {
    if (!wantsPullRequest && !wantsBranch) {
        return undefined;
    }
    if (wantsPullRequest && currentWorkspacePrBranch && currentWorkspacePrUrl) {
        ...
        const branchName = await vscode.window.showInputBox({
            ...
            validateInput: value => {
                ...
                if (cleaned === currentWorkspacePrBranch) {
                    return "A new PR requires a different branch.";
                }
                if (cleaned === GITHUB_BASE_BRANCH || cleaned === "main" || cleaned === "master") {
                    return "Use a feature branch name.";
                }
                return undefined;
            },
        });
        if (!branchName) {
            throw new Error("New pull request creation was cancelled.");
        }
        return createFreshBranchBeforeGeneration(root, prompt, branchName, reporter);
    }
    const context = await resolveGitContext(root);
    const action = await vscode.window.showWarningMessage(`Before applying the infrastructure change, do you want Terrabot to create a clean branch from the latest origin/${GITHUB_BASE_BRANCH}?`, { modal: true }, "Yes - Create New Branch", "No - Use Current Branch");
    ...
    const branchName = await vscode.window.showInputBox({
        ...
        validateInput: value => {
            ...
            if (INVALID_BRANCH_NAME_WORDS.has(cleaned.toLowerCase())) {
                return "Enter a descriptive branch name.";
            }
            if (cleaned === GITHUB_BASE_BRANCH || cleaned === "main" || cleaned === "master") {
                return "Use a feature branch name.";
            }
    ...
    if (!branchName) {
        throw new Error("Branch creation was cancelled because no branch name was provided.");
    }
    return createFreshBranchBeforeGeneration(root, prompt, branchName, reporter);
}
async function readPullRequestTemplate(root) {
...
async function choosePullRequestTarget(prompt, forceSeparate) {
    if (forceSeparate || isSeparatePullRequestPrompt(prompt)) {
        return "new";
    }
    if (isSamePullRequestPrompt(prompt) && currentWorkspacePrBranch) {
        return "existing";
    }
    if (!currentWorkspacePrBranch || !currentWorkspacePrUrl) {
        return "normal";
    }
    const action = await vscode.window.showInformationMessage(`This chat already has a pull request on branch ${currentWorkspacePrBranch}. Where should the latest changes go?`, { modal: true }, "Push Commit to Existing PR", "Create New PR", "Cancel");
    if (action === "Push Commit to Existing PR") {
        return "existing";
    }
...
async function pushBranchWithRecovery(root, branch, notes, reporter, githubToken) {
    try {
        if (githubToken) {
...
async function pushWorkspaceChangesToBranch(root, prompt, reporter, preparedTarget) {
    const notes = [...(preparedTarget?.notes || [])];
    if (!preparedTarget) {
...
async function createPullRequestFromWorkspace(root, prompt, forceSeparate = false, reporter, preparedTarget) {
    const notes = [...(preparedTarget?.notes || [])];
    await abortInterruptedGitOperation(root, notes, reporter);
    ...
    if (status) {
        await runCommand("git", ["add", "-A"], root);
        try {
            await runCommand("git", ["commit", "-m", fallbackCommitTitle], root);
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            if (!/nothing to commit|no changes added/i.test(message)) {
    ...
    const existing = await githubJson("GET", `/repos/${encodeURIComponent(context.owner)}/${encodeURIComponent(context.repo)}/pulls?state=open&head=${encodeURIComponent(head)}&base=${encodeURIComponent(GITHUB_BASE_BRANCH)}`, githubAuth.token);
    let prUrl = "";
    let existed = false;
    if (existing.length) {
        const updated = await githubJson("PATCH", `/repos/${encodeURIComponent(context.owner)}/${encodeURIComponent(context.repo)}/pulls/${existing[0].number}`, githubAuth.token, { title, body });
        prUrl = updated.html_url;
        existed = true;
    }
    else {
        const created = await githubJson("POST", `/repos/${encodeURIComponent(context.owner)}/${encodeURIComponent(context.repo)}/pulls`, githubAuth.token, { title, head: branch, base: GITHUB_BASE_BRANCH, body, draft: true });
...
function explicitIaCPathsFromPrompt(prompt) {
    const found = [];
...
function promptEnvironmentScore(filePath, prompt) {
...
function isAzureResourceCreationPrompt(prompt) {
...
function resourceFamilyTermsFromPrompt(prompt) {
    const text = prompt.toLowerCase();
    const terms = new Set();
...
function resourceFamilyEvidenceScore(filePath, content, prompt) {
    ...
    for (const term of terms) {
        if (normalizedPath.includes(term.replace(/ /g, "_")) || normalizedPath.includes(term.replace(/ /g, "-"))) {
            score += 700;
        }
...
function isTargetEnvironmentCompanionFile(filePath, prompt) {
    if (promptEnvironmentScore(filePath, prompt) <= 0) {
        return false;
    }
...
function routingSymbolsFromPrompt(prompt) {
    const text = prompt.toLowerCase();
    ...
    if (/patch\s*management/.test(text)) {
        symbols.add("create_patch_management");
    }
    if (/diagnostic\s*settings?/.test(text)) {
        symbols.add("create_diagnostic_settings");
    }
    return [...symbols];
}
function repoFileRoutingScore(filePath, content, prompt) {
    let score = 0;
    const normalizedFilePath = filePath.replace(/\\/g, "/").toLowerCase();
    for (const explicitPath of explicitIaCPathsFromPrompt(prompt)) {
        const requested = explicitPath.toLowerCase();
        if (normalizedFilePath === requested) {
            score += 10000;
        }
        else if (normalizedFilePath.endsWith(`/${requested}`)) {
            score += 9000;
        }
    ...
    if (normalizedPath.endsWith("/tier.tfvars")) {
        score += 20;
    }
    return score;
}
...
async function collectRepoContext(root, prompt = "") {
    ...
    const readCandidate = async (uri) => {
        if (cachedContent.has(uri.fsPath)) {
            return cachedContent.get(uri.fsPath);
        }
        try {
    ...
    for (const requested of explicitPaths) {
        ...
        for (const uri of all) {
            ...
            if (rel === normalizedRequested || rel.endsWith(`/${normalizedRequested}`)) {
                addUri(uri);
            }
            else if (!normalizedRequested.includes("/") && rel.split("/").pop() === normalizedRequested) {
                addUri(uri);
            }
        }
    }
    ...
    if (isAzureResourceCreationPrompt(prompt)) {
        ...
        for (const uri of all) {
            ...
            if (familyScore > 0) {
                familyCandidates.push({ uri, score: familyScore });
            }
        ...
        for (const candidate of familyCandidates.slice(0, 16)) {
            addUri(candidate.uri);
        }
        for (const candidate of companionCandidates.slice(0, 12)) {
            addUri(candidate.uri);
        }
    }
    ...
    if (words.length) {
        for (const uri of all) {
            if (picked.size >= MAX_FILES) {
                break;
            }
            const rel = path.relative(root, uri.fsPath).toLowerCase();
            if (words.some(w => rel.includes(w))) {
                addUri(uri);
            }
        }
    }
    ...
    files.sort((left, right) => repoFileRoutingScore(right.path, right.content, prompt) - repoFileRoutingScore(left.path, left.content, prompt));
    return files;
}
// ── virtual document helper (kept for scan / explain-workflow) ───────────────
async function showVirtualDocument(content, language) {
...
async function scanRepository() {
    ...
    if (aiEndpoint) {
        const files = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Terrabot collecting repository context..." }, () => collectRepoContext(root));
        await showVirtualDocument(JSON.stringify({ workspace: root, files }, null, 2), "json");
        return;
    }
    const result = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Terrabot scanning repository..." }, () => runTerrabot(["scan", "--workspace", root, "--json"], root));
    await showVirtualDocument(result, "json");
}
// ── explain-workflow (unchanged) ─────────────────────────────────────────────
async function explainWorkflow(prompt) {
    const root = await getWorkspaceRoot();
    ...
    return vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Terrabot inferring workflow..." }, () => runTerrabot(["explain-workflow", "--workspace", root, "--json", resolvedPrompt], root));
}
// ── Step 7: validation terminal helper ───────────────────────────────────────
async function runValidation(root, commands) {
    if (!commands.length) {
        return;
    }
...
async function applyLiveFileEdit(root, file) {
    ...
    while (prefix < maxPrefix && current[prefix] === finalContent[prefix]) {
        prefix++;
    }
    let suffix = 0;
    ...
    const deleted = await editor.edit(edit => edit.delete(new vscode.Range(startPos, endPos)), { undoStopBefore: true, undoStopAfter: false });
    if (!deleted) {
        return false;
    }
    ...
    for (let offset = 0; offset < newText.length; offset += chunkSize) {
        ...
        const inserted = await editor.edit(edit => edit.insert(pos, chunk), { undoStopBefore: false, undoStopAfter: false });
        if (!inserted) {
            return false;
        }
...
async function applyGeneratedFiles(root, files, summary, validationCommands = [], // Step 7
onWriting, openSourceControl = true) {
    ...
    for (const file of remaining) {
        ...
        if (file.operation === "create") {
            ...
            try {
                ...
                if (normNew.startsWith(normExisting) || existing.trim() === "" || file.in_place || file.typed_insert) {
                    ...
                    vscode.window.showErrorMessage(`Terrabot blocked an unsafe edit for ${cleanedPath}: generated content was not a prefix-preserving append or typed insert. No duplicate code was appended.`);
                }
            }
            catch {
    ...
    const action = await vscode.window.showInformationMessage(`Terrabot applied ${files.length} file(s). ${summary}`, "Run Validation", "Dismiss");
...
async function askInfrastructure(prompt) {
    ...
    const result = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Terrabot generating changes..." }, async () => postJson(aiEndpoint, {
        prompt: resolvedPrompt,
        workspace_name: path.basename(root),
    ...
    if (!files.length) {
        vscode.window.showInformationMessage(result.reply || result.summary || "No files were generated.");
        return;
    }
    await applyGeneratedFiles(root, files, result.summary || result.reply || "", result.validation_commands || []);
}
...
function registerChatParticipant(context) {
    ...
    const participant = chatApi.createChatParticipant("terrabot.infra", async (request, chatContext, stream) => {
        const prompt = request.prompt || "";
        const command = request.command || "plan";
        if (!prompt) {
        ...
        try {
            ...
            if (wantsPullRequest && isPurePullRequestPrompt(prompt)) {
                ...
                const created = await createPullRequestFromWorkspace(root, prompt, forceSeparate, (message) => stream.progress(message));
            ...
            const preparedGitTarget = await prepareGitTargetBeforeGeneration(root, prompt, wantsPullRequest, wantsBranch, (message) => stream.progress(message));
            const aiEndpoint = getAiEndpoint();
            ...
            const historyLength = (chatContext && chatContext.history)
            ...
            if (analysis) {
                stream.markdown("*Terrabot's analysis:*\n\n" +
                    analysis.split("\n").map((l) => `> ${l}`).join("\n") +
                    "\n\n");
            }
            if (!files.length) {
                // Chat / repo Q&A / clarification path — render the agent's words directly.
                const replyText = result.reply || result.summary || "";
                if (replyText) {
            ...
            await applyGeneratedFiles(root, files, result.summary || result.reply || "", result.validation_commands || [], (filePath) => stream.progress(`Terrabot is writing ${filePath}...`), !(wantsPullRequest || wantsBranch));
            // Report back in the chat panel.
            const fileList = files
                .map(f => `- \`${f.path}\` (${f.operation})`)
                .join("\n");
            stream.markdown(`**Applied ${files.length} file(s)** to your workspace:\n\n${fileList}\n\n` +
                `${result.summary || ""}\n\n` +
            ...
            if (wantsPullRequest) {
                ...
                const forceSeparate = isSeparatePullRequestPrompt(prompt);
                const created = await createPullRequestFromWorkspace(root, prompt, forceSeparate, (message) => stream.progress(message), preparedGitTarget);
                await vscode.commands.executeCommand("workbench.view.scm");
                ...
                const pushed = await pushWorkspaceChangesToBranch(root, prompt, (message) => stream.progress(message), preparedGitTarget);
                await vscode.commands.executeCommand("workbench.view.scm");
            ...
            const fillable = result.user_fillable || [];
            if (fillable.length) {
            ...
            stream.markdown(`Terrabot failed: ${err instanceof Error ? err.message : String(err)}`);
        }
    });
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("terrabot.scanRepository", scanRepository), vscode.commands.registerCommand("terrabot.explainWorkflow", async () => showVirtualDocument(await explainWorkflow(), "json")), vscode.commands.registerCommand("terrabot.askInfrastructure", () => askInfrastructure()), vscode.commands.registerCommand("terrabot.connectGitHub", async () => {
        const auth = await connectGitHubAccount();
        vscode.window.showInformationMessage(`Terrabot connected to GitHub as ${auth.account}.`);
    }), vscode.commands.registerCommand("terrabot.disconnectGitHub", disconnectGitHubAccount), vscode.commands.registerCommand("terrabot.openStandaloneChat", async () => {
        const prompt = await vscode.window.showInputBox({
            title: "Terrabot",
            prompt: "Describe the infrastructure change you want.",
            placeHolder: "Example: Disable patch management in sbx-infra",
            ignoreFocusOut: true,
        });
        if (!prompt?.trim()) {
            return;
